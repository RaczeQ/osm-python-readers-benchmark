...
def osmnx_download_buildings(
    directory: Path, geometry: BaseGeometry, **kwargs: Any
) -> int:
    """Download cache and return total size."""
    osmnx.settings.cache_folder = directory
    _download_overpass_features(geometry, {"building": True})

    return sum(p.stat().st_size for p in osmnx.settings.cache_folder.glob("**/*"))


def osmnx_get_buildings(geometry: BaseGeometry, **kwargs: Any) -> None:
    features_from_polygon(polygon=geometry, tags={"building": True})
